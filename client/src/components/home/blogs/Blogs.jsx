...
const Blogs = () => {
  ...
  const { filters, searchs, pagination } = useSelector((state) => state.blog.blogQueries);
  ...
  let queryString = generateQueryPublished({ filters, searchs, pagination });
  console.log('Blogs',filters, searchs, pagination);
  
 

  useEffect(() => { 
    getBlogsApi(queryString);
    getCommentsApi();
